...
First, note that this repository is intended to teach my son (currently age 9) how to use the command line, so I am aware that my wording may seem a little basic for adults that have a higher reading level.
...
## The Command Line:
To open the terminal, you can search in the "Applications" folder or something similar and find a program called "terminal" or find an icon that looks like this: ![](Images/Mac_Terminal_Icon.png)
...
## Mouse Clicks and Case Sensitivity
When you open the command line, you will notice that you can't click on files or directories anymore. It is different from Finder or Window's Explorer in that regard. This can be frustrating at first, but you will find ways to work without the mouse using the command line. One benefit this gives is that you can highlight things to copy a path or file name without losing your cursor position (where you are typing). Another important thing to note is that the command line is case sensitive. That means if a file is called "Classes.txt" and you accidentally type in "classes.txt", it will not recognize what you have just typed.

## Tab completion
One way to make typing a little easier on the command line is to use the tab key. This key is located on the left side of the keyboard above the caps lock button. If you start typing the name of a file or directory and no other files begin with the letters that you have typed, you can hit tab and it will complete the typing. If there are other files or directories that have the same first few letters, it will type all of the letters until it comes to the place where the names differ and it will allow you to type the next character to differentiate. This can be particularly helpful when you have long names and you are likely to have a typo if you type the entire thing. 
